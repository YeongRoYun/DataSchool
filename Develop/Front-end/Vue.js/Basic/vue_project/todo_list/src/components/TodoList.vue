<template>
    <!--main tag contains "central" context-->
    <main>
        <div v-for="(todo, id) in data" :key="todo.id">
            <div class="input-group my-2 input-group-sm">
                <div class="input-group-text">
                    <input class="form-check-input mt-0" type="checkbox"
                    :checked="todo.completed" :disabled="todo.completed"
                    @click="completeTodo(todo.id)"/>
                </div>
                <div class="input-group-text">
                    <input class="form-input mt-0" type="date" :min="today" disabled
                    :value="todo.date" />
                </div>
            ...
            <div v-show="id + 1 < data.length" class="col border border-second" />
        </div>
    </main>
</template>
